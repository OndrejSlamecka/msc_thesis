...
and loss, respectively. \autoref{fig:example_game} depicts such a game
in a standard form -- using a tree.
...
\begin{example}
    In \autoref{fig:goal_opt} let Alice be player 1 and Bob player 2.
...
\end{tikzpicture}
\end{center}
\caption{A single turn, two player game.}
\label{fig:goal_opt}
\end{figure}
...
\Function{negamax}{$state, player$}
    ...
    $[1,-1][player]$ part returns 1 for player 0 but $-1$ for player 1.}
